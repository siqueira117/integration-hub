...
class AuxValidator
{
    ...
    private function validateCNAE($cnae)
    {
        if (empty($cnae)) {
            return false;
        }
    ...
    private function validateCPF($cpf, $applyRegexToRemoveMask = true)
    {
    ...
    private function validateDDD($ddd)
    {
    ...
    private function validateCelular($cel)
    {
    ...
    private function validateCAEPF($caepf)
    {
    ...
    private function validate_emaildomain($email): bool
    {
    ...
    private function validateEmailDomainMXrecord($email): bool
    {
        $parts = explode("@", $email);
        if (count($parts) == 2) {
    ...
    private function validateEmailDomain($email): bool
    {
        $parts = explode("@", $email);
        if (count($parts) == 2) {
            $domain = array_pop($parts);
